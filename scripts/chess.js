            ...
            if (type === "B") {
                ...
                for (i = 0; i < moves.length; i += 1) {
					...
					while (board[b] !== undefined && board[b][a] !== undefined) {
                        if (clear === true) {
                            if (board[b][a].piece === 0) {
                                legalPath[legalPath.length] = {x: convertX[a], y: b};
				...
				if (board[y] === undefined) {
					...
					legalPath[legalPath.length] = {x: convertX[x + a], y: y};
				}
				if (!hasMoved && board[y][x + 2 * a].piece === 0) {
					legalPath[legalPath.length] = {x: convertX[x + 2 * a], y: y};
				}
				if (board[y + 1] === undefined) {
					...
					legalPath[legalPath.length] = {x: convertX[x + a], y: (y + 1)};
				}
				if (board[y - 1] === undefined) {
				} else if (board[y - 1][x + a] === undefined) {
				} else if (board[y - 1][x + a].piece === 0) {
...
function clicked(almostX, y) {
    ...
    if (currentPath) {
        for (i = 0; i < currentPath.length && !isPathSquare; i += 1) {
            isPathSquare = currentPath[i].x + currentPath[i].y === (almostX + y);
		}
		if (isPathSquare) {
		...
		if (currentPath) {
            for (i = 0; i < currentPath.length; i += 1) {
                d.getElementById(currentPath[i].x + currentPath[i].y).style.backgroundColor = "white";
            }
        }
    }
	return;
